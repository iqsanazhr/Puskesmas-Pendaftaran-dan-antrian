            ...
            <div class="p-8">
                <div class="flex flex-col md:flex-row justify-between items-center mb-10 gap-4">
                    ...
                    <div class="w-full md:w-72">
                        <label for="poly-select" class="block text-sm font-medium text-slate-700 mb-1">Pilih Poli</label>
                ...
                @if($selectedPolyId)
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
                        ...
                        <div class="bg-gradient-to-br from-blue-50 to-white p-8 rounded-2xl border border-blue-100 shadow-sm flex flex-col items-center justify-center text-center relative overflow-hidden">
                            <div class="absolute top-0 right-0 -mt-4 -mr-4 w-24 h-24 bg-blue-100 rounded-full opacity-50 blur-xl"></div>
                            <div class="absolute bottom-0 left-0 -mb-4 -ml-4 w-32 h-32 bg-medical-teal/10 rounded-full opacity-50 blur-xl"></div>
                            ...
                            @if($currentQueue)
                                <div class="relative z-10">
                                    ...
                                    <div class="text-lg font-medium text-slate-700 mb-8 bg-white/60 px-4 py-1 rounded-full inline-block backdrop-blur-sm border border-blue-100">
                                        {{ $currentQueue->patient->name }}
                                    </div>
                                    
                                    <button wire:click="completeCurrent"
                                        class="group bg-green-500 text-white px-8 py-3 rounded-full font-semibold hover:bg-green-600 transition-all duration-200 shadow-lg shadow-green-500/30 flex items-center gap-2 transform hover:-translate-y-1">
                                            ...
                                            <li class="p-4 hover:bg-slate-50 transition duration-150 group">
                                                <div class="flex justify-between items-center">
                                                    <div class="flex items-center gap-4">
                                                        <span class="bg-slate-100 text-slate-600 font-bold text-lg w-10 h-10 rounded-full flex items-center justify-center group-hover:bg-medical-blue group-hover:text-white transition-colors">
                                                            {{ $queue->number }}
                                                        </span>
                                                        <div>
                                                            <p class="font-semibold text-slate-900">{{ $queue->patient->name }}</p>
                                                            <p class="text-xs text-slate-500">Terdaftar: {{ $queue->created_at->format('H:i') }}</p>
                                                        </div>
                                                    </div>
                                                    <span class="px-3 py-1 bg-yellow-100 text-yellow-700 text-xs font-medium rounded-full border border-yellow-200">
                                                        Menunggu
                                                    </span>
                                                </div>
                                            </li>
                ...
                @else
                    <div class="text-center py-20 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200">
                        <div class="inline-block p-6 bg-white rounded-full shadow-sm mb-4">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-12 w-12 text-medical-blue" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
                            </svg>
                        </div>
                        <h3 class="text-lg font-semibold text-slate-900">Pilih Poli Terlebih Dahulu</h3>
                        <p class="text-slate-500 mt-2 max-w-sm mx-auto">Silakan pilih poli tujuan pada menu dropdown di atas untuk mulai mengelola antrian pasien.</p>
                    </div>
                @endif
            </div>
        </div>
